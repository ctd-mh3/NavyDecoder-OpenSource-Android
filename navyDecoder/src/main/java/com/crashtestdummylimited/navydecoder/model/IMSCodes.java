...
public class IMSCodes implements ReferenceData {
  ...
  public IMSCodes() {
    String[][] CODE_MEANING_DATA = {
      ...
      {
        "RC2.5",
        "Mobilizing Reservist completed all admin, medical, dental, and training requirement"
      },
      ...
      {
        "RU2",
        "NRA unable to verbally contact Reservist identified for mobilization/recall after 30 days"
      },
      {
        "RUA",
        "NRA verbally contacted Reservist identified for mobilization/recall, but Reservist did not report as ordered."
      },
      ...
      {
        "EMN",
        "Not physically qualified for mobilization. (MRR package submitted and/or CNRFC N35 directed use)"
      },
      {"EMD", "Deceased"},
    ...
    for (String[] row : CODE_MEANING_DATA) {
      mIMSCodesHashMap.put(row[0], row[1]);
    }
    String[] keys = mIMSCodesHashMap.keySet().toArray(new String[0]);
    Arrays.sort(keys);
    mSortedKeys = keys;
  }
